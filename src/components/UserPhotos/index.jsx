...
import { IconButton, Typography, Grid, Card, CardMedia, TextField, CardContent, Button, Divider, Box, List, Paper, Chip, Snackbar, Alert} from "@mui/material";
...
function UserPhotos ({advancedFeature, setAdvancedFeature}) {
    ...
    useEffect(() => {
      ...
      fetchData();
    }, [userId, photoId, currentPhotoIndex, setAdvancedFeature, advancedFeature, navigate]);
    ...
    const handleClickConfirm = async (comment_id) => {
      try {
        const response = await fetch(BE_URL + "/api/comment/commentsOfPhoto/" + photoId, {
          method: "PATCH",
          headers: {
        ...
        <Card>
          ...
          <CardContent>
            ...
            <Grid container spacing={2} alignItems="center">
              ...
              <Grid item xs={2}>
                <Button variant="contained" fullWidth onClick={handleAddCommentClick} disabled={!newComment || newComment.trim() === ''}>
                  <AddIcon/>
                  Add comment
                </Button>
              </Grid>
                ...
                {currentPhoto.comments.map((comment) => (
                  <Paper key={comment._id} sx={{mb: 2, p: 2}}>
                    ...
                    <Grid container spacing={2} alignItems="center">
                      ...
                      <Grid item xs={2}>
                        {!isEdit[comment._id] ? (
                          <Button variant="contained" fullWidth onClick={() => handleClickEdit(comment._id, comment.comment)}>
                            <EditIcon />
                              Edit
                            </Button>
                        ):(
                          <Button variant="contained" fullWidth onClick={() => handleClickConfirm(comment._id)}>
                            Confirm
                          </Button>
                        )}
